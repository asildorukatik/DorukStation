...
(function(root,factory){
  ...
  root.DorukStationV57Store=api;
  if(typeof window!=='undefined'&&typeof document!=='undefined'){
    try{api.installBrowser()}catch(err){console.error('[DorukStation Store v0.57]',err)}
  }
  ...
  function validatePackageEntries(game,entries){
    if(!(entries instanceof Map))throw new Error('Package entries must be a Map');const manifestBytes=entries.get('manifest.json');if(!manifestBytes)throw new Error('Package is missing manifest.json');let manifest;try{manifest=JSON.parse(new TextDecoder().decode(manifestBytes))}catch{throw new Error('Package manifest.json is invalid JSON')};const expected=slug(game?.id||game?.name),actual=slug(manifest?.id||expected);if(actual!==expected)throw new Error(`Package manifest id mismatch: expected ${expected}, got ${actual}`);const entry=String(manifest?.entry||game?.entry||'Game/index.html').replace(/\\/g,'/');if(!isSafePackagePath(entry)||!entries.has(entry))throw new Error(`Package entry is missing or unsafe: ${entry}`);if(!/\.html?$/i.test(entry))throw new Error('Package entry must be an HTML file');return {...manifest,id:expected,entry};
  }
  ...
  function renderStoreHomeMarkup(state,modern=false){
    const games=state?.catalog?.games||[],featured=games.find(g=>g.featured)||games[0],cards=games.map((g,i)=>gameCardMarkup(state,g,i,modern)).join('');
    if(modern)return `<section class="v57-store v57-store-modern"><div class="v57-modern-store-head"><div><span>DorukStation Store</span><h1>${htmlEscape(featured?.name||'Discover Games')}</h1></div><button data-store-action="search" type="button">⌕ Search</button></div>${featured?`<div class="v57-modern-hero" style="background-image:linear-gradient(90deg,rgba(4,5,10,.94),rgba(4,5,10,.18)),url('${attrEscape(featured.banner||featured.icon)}')"><div><span>${htmlEscape(featured.age)}</span><h2>${htmlEscape(featured.name)}</h2><p>${htmlEscape(featured.description||'Available from DorukStation Store.')}</p></div></div>`:''}<div class="v57-modern-actions"><button data-store-action="home">Featured</button><button data-store-action="search">Search</button><button data-store-action="downloads">Downloads</button><button data-store-action="storage">Storage</button></div><h3>Featured</h3><div class="v57-store-grid">${cards||'<div class="v57-store-empty">Catalog is empty.</div>'}</div>${renderSystemFooterMarkup(state,'✕ Select · ○ Back · OPTIONS Details')}</section>`;
    return `<section class="v57-store v57-store-classic"><aside class="v57-store-rail"><div class="v57-store-brand">DorukStation Store</div><button data-store-action="home" class="active">What's Hot</button><button data-store-action="search">Store Search</button><button data-store-action="downloads">Downloads</button><div class="v57-store-rule"></div><span>Browse</span><button data-store-filter="featured">Popular</button><button data-store-filter="adventure">Adventure</button><button data-store-filter="racing">Racing</button><button data-store-filter="horror">Horror</button><button data-store-filter="strategy">Strategy & Puzzles</button><button data-store-filter="building">Building</button><button data-store-filter="fight">Fight</button><div class="v57-store-rule"></div><button data-store-action="storage">Storage Settings</button></aside><main class="v57-store-main">${featured?`<div class="v57-classic-feature"><div class="v57-feature-title">Most Popular</div><div class="v57-feature-hero" style="background-image:linear-gradient(90deg,rgba(0,41,116,.18),rgba(0,20,78,.2)),url('${attrEscape(featured.banner||featured.icon)}')"><div><h1>${htmlEscape(featured.name)}</h1><p>${htmlEscape(featured.description||featured.genres.join(' · '))}</p></div></div></div>`:''}<h2>Most Popular</h2><div class="v57-store-grid">${cards||'<div class="v57-store-empty">Catalog is empty. Upload catalog.json to the Game Library repo.</div>'}</div></main>${renderSystemFooterMarkup(state,'✕ Enter · ○ Back · OPTIONS Menu')}</section>`;
  }
  ...
  function installBrowser(){
    ...
    window.__dorukstationVersion='0.57';
    document.title='DorukStation — v0.57';
    const v57ShowUserSelectorBase=typeof showUserSelector==='function'?showUserSelector:null;
    if(v57ShowUserSelectorBase)showUserSelector=function(...args){const out=v57ShowUserSelectorBase(...args);document.title='DorukStation — v0.57';return out};
    ...
    function bindStoreBody(body,view,gameId=''){
      ...
      body.querySelectorAll('[data-store-filter]').forEach(el=>el.addEventListener('click',()=>{runtime.search=el.dataset.storeFilter==='featured'?'':el.dataset.storeFilter;openStoreSearch(runtime.search)}));
